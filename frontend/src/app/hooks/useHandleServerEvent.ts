...
// import handleAudioLoop from "@/app/agentConfigs/chatSupervisor/handleAudioLoop";

import { isWaitingForSupervisorRef,
  holdMusicTimeoutRef,
  holdMusicStartedAtRef, } from "@/app/contexts/sharedRefs";
import handleAudioLoop from "@/app/agentConfigs/chatSupervisor/handleAudioLoop"; 
...
export function useHandleServerEvent({
  ...
  const handleServerEvent = (serverEvent: ServerEvent) => {
    ...
    switch (serverEvent.type) {
      ...
      // Play hold music only if waiting for supervisor response
      // if (isWaitingForSupervisorRef.current) {
      //   handleAudioLoop({ action: "play" }).catch(console.error);
      // }

      if (isWaitingForSupervisorRef.current) {
      // ⏳ Start a 4s timer — defer hold music until 4s after agent finishes talking
        holdMusicTimeoutRef.current = setTimeout(async () => {
          if (isWaitingForSupervisorRef.current) {
            await handleAudioLoop({ action: "play" });
            holdMusicStartedAtRef.current = Date.now();
          }
        }, 3000);
      }
